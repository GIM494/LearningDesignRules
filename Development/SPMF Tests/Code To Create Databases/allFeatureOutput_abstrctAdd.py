...
attributeList = {}
# List of parent classes so we can understand more about the
# parent-child relationships in the class
parentClassList = []
...
def generateAttributes(classRootNode, id_start):
    ...
    if classExtends != None:
        clsName = (classExtends.find('src:name', nameSpace))
        if clsName.text is None:
            clsName = (clsName.find('src:name', nameSpace)).text
        else:
            clsName = clsName.text
        if clsName is not None:
            classExtendsName = "class extends class " + clsName
            if classExtendsName not in attributeList:
                attributeList[classExtendsName] = id_no
                id_no += 1
            if clsName not in parentClassList:
                parentClassList.append(clsName)
    ...
    return id_no, attributeList, parentClassList
...
def outputDatabase(peripheralClassRoot, allAttributes, fileName):
    #print("Going to output databse!!")


    #print(peripheralClassRoot)


    # Class id for outputting to match format needed for matrix
    # factorization
    class_id = 1
    ...
    #print(len(peripheralClassRoot.findall(".//src:class", nameSpace)))
    for cl in peripheralClassRoot.findall(".//src:class", nameSpace):

        #print("Doing things for class")
        #print(cl)
        ...
        # Find attributes having to do with parent/child relationships when
        # the parent is an abstract class

        #print("Just before target function call")
        attributes = PCR.findParentChildRelations(peripheralClassRoot, allAttributes, attributes, parentClassList)
